...
> 补充：在`calc()`中使用scss变量需要使用`#{}`语法，要不然编译不成功，并且项目打包报错
>
> ```scss
> $height: 70px;
> 
...
~~~scss
$name: 'Tom Bob';
$container: "top bottom";//中间有空格，就加上引号
$what: heart;
...
Maps必须被**圆括号包围**，可以映射任何类型键值对（任何类型，包括内嵌maps，不过不推荐这种内嵌方式）
...
calc(100% - 200px)  两个数之间必须要`加上空格`，否则编译通过但是会有错误。
